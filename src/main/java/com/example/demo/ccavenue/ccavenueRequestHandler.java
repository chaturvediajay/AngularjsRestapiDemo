...
public static void main(String[] args) {
	...
	private boolean saveOrder(Map<String, String> values, HttpServletRequest request) {
		...
		if (values.get("txnid") != null & !values.get("txnid").isEmpty()) {
			com.example.demo.model.LoginSession ls = (com.example.demo.model.LoginSession) request.getSession().getAttribute("loginSession");
			com.example.demo.model.ShippingModel sm = (com.example.demo.model.ShippingModel) request.getSession().getAttribute("shippingModel");
			sm.setOrderId(values.get("udf1"));
			sm.setUid(ls.getId());
			bol = com.example.demo.logic.CartLogic.orderDetailSave(sm, values.get("udf1"), request);
			com.example.demo.cart.PaymentDetails pd = new com.example.demo.cart.PaymentDetails();
