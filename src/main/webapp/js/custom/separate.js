...
function cartview(response) {
	...
	if (da != null & da.length>5 ) {
			...
			$('#test')
					.append(
							"<div class=\"media\">"
									...
									+ object['smrp']
									+ '&#61;'
									+ object['qty'] * object['smrp']
									+ "</div>"
									+ "</div>"
									+ "<div class=\"media-right\"><a href=\"#\" data-toggle=\"tooltip\" "
									+ "onclick=\"act(this,0,'"
									+ object['pkey']
									+ object['ppid']
									+ "','delete','jq');\" title=\"Remove\"><i class=\"fa fa-remove\"></i></a></div>"
									+ "</div>");
		...
		$('#test')
				.append(
						"<div class=\"subtotal-cart\">Subtotal: <span>&#x20a8; "
								+ amt
								+ ".00</span></div>"
								+ "<div class=\"text-center\">"
								+ "<div class=\"btn-group\" role=\"group\" aria-label=\"View Cart and Checkout Button\">"
								+ "<a class=\"btn btn-default btn-sm\" href=\"cart\"><i class=\"fa fa-shopping-cart\"></i> View Cart</a>"
								/*
...
var selectGet = function(id, da, msgId) {
	...
	if (da != null) {
		for (var i = 0; i < da.length; i++) {
				mySelect.append('<option value="'+da[i].ppid+'">'+da[i].color+'</option>');
		}
